...
model_path="../model/dorami"
device=torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
logging.info(f"use device:{device}")

def create_tokenizer():
    bertTokenizer = BertTokenizerFast.from_pretrained(tokenizer_name)
...
def create_model(tokenizer):
    config = GPT2Config.from_pretrained(model_name, vocab_size=tokenizer.vocab_size, n_positions=max_len, n_ctx=max_len, bos_token_id=tokenizer.bos_token_id, eos_token_id=tokenizer.eos_token_id)
